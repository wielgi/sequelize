...
class QueryGenerator {
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    if (
      ...
      if (
        ...
        onDuplicateKeyUpdate += _.isEmpty(updateKeys)
          ? " DO NOTHING "
        ...
        if (_.isEmpty(valueKeys) && options.upsertKeys) {
          valueKeys.push(
            ...options.upsertKeys.map(
              (attr) =>
                `${this.quoteIdentifier(attr)}=${this.quoteIdentifier(attr)}`
            )
          );
        }
  ...
  */
  ...
  getConstraintSnippet(tableName, options) {
    ...
    switch (options.type.toUpperCase()) {
      ...
      case "FOREIGN KEY":
        ...
        if (
          !references ||
          !references.table ||
          !(references.field || references.fields)
        ) {
          throw new Error(
    ...
    if (
      options.deferrable &&
      ["UNIQUE", "PRIMARY KEY", "FOREIGN KEY"].includes(
  ...
  */
  ...
  quoteIdentifier(identifier, force) {
    throw new Error(
      `quoteIdentifier for Dialect "${this.dialect}" is not implemented`
    );
  }
  ...
  quoteTable(param, alias) {
    ...
    if (_.isObject(param)) {
      ...
      if (this._dialect.supports.database && param.schema.includes("."))
        table += `[${param.schema.split(".")[0]}].`;

      if (this._dialect.supports.schemas) {
        if (param.schema) {
          if (param.schema.includes(".")) {
            // If there is a dot in schema - get part after dot
            table += `${this.quoteIdentifier(param.schema.split(".")[1])}.`;
          } else {
            // no changes when theres no dot
  ...
  selectQuery(tableName, options, model) {
    ...
    if (options.lock && this._dialect.supports.lock) {
      ...
      if (
        ...
        ["KEY SHARE", "NO KEY UPDATE"].includes(lock)
      ) {
        query += ` FOR ${lock}`;
      } else if (lock === "SHARE") {
    ...
    if (
      ...
      if (parentIsTop) {
        ...
        if (topLevelInfo.subQuery) {
          const dbIdentifier = `${tableName}.${this.quoteIdentifier(
            fieldLeft
          )}`;
          subqueryAttributes.push(
            dbIdentifier !== joinOn
              ? `${dbIdentifier} AS ${this.quoteIdentifier(attrLeft)}`
              : dbIdentifier
          );
        }
      } else {
